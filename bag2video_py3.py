...
def get_info(bag, topic=None, start_time=rospy.Time(0), stop_time=rospy.Time(2**32)):
    size = (0,0)
    times = []
...
def write_frames(bag, writer, total, topic=None, nframes=repeat(1), start_time=rospy.Time(0), stop_time=rospy.Time(2**32), viz=False, encoding='bgr8'):
    bridge = CvBridge()
    if viz:
        cv2.namedWindow('win')
    count = 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--end', '-e', action='store', default=rospy.Time(2**32), type=rospy.Time,
